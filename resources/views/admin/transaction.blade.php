    ...
    <div class="overflow-x-auto mb-8">
        <table class="table  bg-white">
            ...
            <tbody>
                ...
                <td>{{ $per_transaction->status }}</td>
                @if($index === 0)
                <td class="flex gap-2" rowspan="{{ count($transaction) }}">
                    <a href="{{ route('admin.transaction.print',['date' => $date,'user_id' => $per_transaction->user->id] )}}" class="btn btn-error text-white"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                            height="16" fill="currentColor" class="bi bi-file-earmark-pdf-fill" viewBox="0 0 16 16">
                            <path
